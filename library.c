...
extern void m2CDB_ChuckieEggLogo(void);
extern void m2D3A(void);
extern void m2D7E_checkForMenuKeyPress(void);
...
m192B:
  ASL_ZEROP(0x84)
  BCC(m1935)
  LDA_POSTI(0x70) /* Read from (screen_address + Y) */
  EOR_ZEROP(0x82) /* Toggles the bottom bit?*/
  STA_POSTI(0x70) /* Store it back (screen address is stored in 0x70(lo) & 0x71(hi) in zero page memory ). Add Y as offset. */
m1935:
  DEC_ZEROP(0x83)
  BNE(m194B)
  INC_ZEROP(0x76)
...
/* This is the main game-loop time delay to set the speed of the game. */
/* Seems to wait until 300ths of a second has elapsed. */
void m1B10(void)
{
...
//m1B10:
  ...
  OSWORD
  LDA_ZEROP(0x01)
  /* Overflowed into MSB, so reset interval timer and exit. */
  BNE(m1B23)
  LDA_ZEROP(0x00)

  // If more than 2ms to wait, then send thread to sleep
/*  timeleft = (r6502_a*10);
  if(timeleft > 2)
  {
      sleep(timeleft - 1);
...
m1B23:
  ...
  /* Poll needs to be called at least once a frame to ensure keyboard input is handled. */
  PollMessages();
}
#if 0
void m1B10(void)
...
void m1B33(void)
{
  /* Clear graphics area (CLG) VDU 16*/
  LDA_IMMED(0x10)
  OSWRCH
...
m2897:
  ...
  BEQ(m28BD)
  LDA_ABSOL(0x295C)
  /* Subtract 48 from y graphics cursor */
  SEC
  SBC_IMMED(0x30)
  STA_ABSOL(0x295C)
  LDA_ABSOL(0x295D)
...
void m2ACD_systemInit(void)
{
  /* (*fx 200,2) - Read/write ESCAPE, BREAK effect i.e. disable ESCAPE and clear memory on BREAK.  */
  LDA_IMMED(0xC8)
  LDX_IMMED(0x02)
  LDY_IMMED(0x00)
  OSBYTE
...
m2C28:
  ...
  m2E2D();
  /* Restore default graphics windows */
  LDA_IMMED(0x1A)
  OSWRCH
  /*RTS*/
}
...
void m2C54(void)
{
  /*delay loop used during "Get Ready Player #" message */
  STA_ZEROP(0x8A)
m2C56:
  LDY_IMMED(0x00)
  LDX_IMMED(0x00)
/*m2C5A:*/
  ...
  /*slow down*/
  /* BBC B 6502A is clocked at 2Mhz*/
  /* The inner loop takes 5 cycles to execute and loops round 256 times. */
  /* So one iteration of the outer loop takes (256*5) cycles plus an extra five cyles to execute its DEY and BNE. */
  /* 256*(256*5 + 5) = 328960 cycles = 0.16448 seconds*/
  delay(328960/2000); /* delay accepts milliseconds */
  PollMessages();
...
/*m2C6B:*/
  do
  {
  /* Clear graphics area - CLG */
  LDA_IMMED(0x10)
  OSWRCH
  /*JSR_ABSOL(0x2CDB,m2CDB)*/
  m2CDB_ChuckieEggLogo();
  /*JSR_ABSOL(0x2838,m2838)*/
  m2838_displayHighScores();
...
/* Does this display the Chuckie Egg title bar?*/
void m2CDB_ChuckieEggLogo(void)
{
  LDA_IMMED(0x02)
  STA_ZEROP(0x7F)
  LDA_IMMED(0x30)
...
void m2D7E_checkForMenuKeyPress(void)
{
  PollMessages();
...
/* Delay used between demo screens*/
void m2DA6(void)
{
  /*delay timer*/
  /*JSR_ABSOL(0x2DA9,m2D7E)*/
  m2D7E_checkForMenuKeyPress();
  if (showscreens)
  {
    LDX_IMMED(0x00)
    LDY_IMMED(0x00)
/*m2DAD:*/
    ...
    /*slow down*/
    /* BBC B 6502A is clocked at 2Mhz*/
    /* The inner loop takes 5 cycles to execute, so */
    /* 256*(256*5 + 5) = 328960 cycles = 0.16448 seconds*/
    delay(328960/2000);
    PollMessages();
  }
...
m2F88:
  ...
  timeleft += ((duration*1000)/20); /* duration is in 20ths of a second */

  DEC_ZEROP(0x8A)
  BNE(m2F88)
  /*RTS*/

  /* Wait for death tune to finish playing. */
  delay(timeleft);
  PollMessages();
}
...
m305C:
  ...
  LDA_IMMED(0x79)
  LDX_IMMED(0x80) /* 0x80 ^ 0x80 = 0x00 - check for internal key pressed (key 0 i.e. SHIFT)*/
  OSBYTE /* neagtive if key pressed */
  TXA
  BPL(m306B)
  LDX_IMMED(0x00)
  JMP_ABSOL(m3083)
m306B:
  ...
  LDA_IMMED(0x79)
  LDX_IMMED(0x81) /* 0x81 ^ 0x80 = 0x01 - check for internal key pressed (key 1 i.e. CTRL) */
  OSBYTE /* neagtive if key pressed */
  TXA
  BPL(m307A)
  LDX_IMMED(0x01)
  JMP_ABSOL(m3083)
m307A: /* neither shift nor ctrl were pressed, so start scan from internal key 16 onwards. */
  LDA_IMMED(0x7A) /* Keyboard scan from 16 decimal */
  OSBYTE7A
  CPX_IMMED(0xFF) /* 0xff indicates that no key was scanned as being pressed. */
  BEQ(m305C) /* loop round and repeat tests until a key is pressed. */
m3083:
  STX_ZEROP(0x88) /* x = 0 if shift was pressed and x = 1 if ctrl was pressed. */
  TXA
  EOR_IMMED(0xFF)
  STA_ZEROP(0x89)
  LDY_ZEROP(0x60)
...
m30B8:
  ...
  LDA_ZEROP(0x8E)
  BEQ(m30D4)
  /* Set BELL duration and play bell sound. */
  LDA_IMMED(0xD6)
  LDX_IMMED(0x01)
  LDY_IMMED(0x00)
  OSBYTE
